...
--Admin Components Procedures (Basel Mohamed)
go
Alter Proc AdminIssueInstallPayment
@paymentID int,
@InstallStartDate date
As
if(exists(select * from Payment where id=@paymentID))
...
go
select paymentId from Installment
go

create proc availableThesis
as
select serialNumber
from Thesis
go
...
-- Student Components Procedures (Mahmoud Moamen)
go
create proc addPublication
@title varchar(50), @pubDate datetime, @host varchar(50), @place
varchar(50), @accepted bit, @PUBID int output
as
insert into Publication values(@title,@pubDate,@place,@accepted,@host)
Set @PUBID=(Select id from Publication where title=@title and dateOfPublication=@pubDate and place=@place 
and accepted=@accepted and host=@host)
go
...
create proc ExaminerRegister 
	@ExaminerName varchar(20), @email varchar(50),@Password varchar(30),@FieldOfWork varchar(10),@Nationality bit
as
	insert into PostGradUser values(@email,@Password)
	declare @id int
	set @id = SCOPE_IDENTITY()
	insert into Examiner values(@id,@ExaminerName,@fieldOfWork,@Nationality)
...
declare @gucian bit
if(exists(
select * from GucianStudent inner join GUCianStudentRegisterThesis on GucianStudent.id=GUCianStudentRegisterThesis.sid
inner join Thesis on GUCianStudentRegisterThesis.serial_no=Thesis.serialNumber
where id = @studentID and Thesis.endDate<=CURRENT_TIMESTAMP
))
begin
set @gucian = '1'
end
else
select * from NonGucianStudent inner join NonGUCianStudentRegisterThesis on NonGucianStudent.id=NonGUCianStudentRegisterThesis.sid
...
select id from NonGucianStudent inner join NonGUCianStudentRegisterThesis on NonGucianStudent.id=NonGUCianStudentRegisterThesis.sid
inner join Thesis on NonGUCianStudentRegisterThesis.serial_no=Thesis.serialNumber
where id = @studentID and Thesis.endDate<=CURRENT_TIMESTAMP
begin
set @gucian = '0'
...
@studentID int
as
select NonGucianStudentTakeCourse.grade,Course.fees,Course.creditHours,Course.code
...
go
create proc ListAllmyTheses
	@id int
as
if(exists(Select Th.* from GUCianStudentRegisterThesis Gs inner join Thesis Th on Gs.serial_no=Th.serialNumber
where Gs.sid=@id))
begin
Select Th.* from GUCianStudentRegisterThesis Gs inner join Thesis Th on Gs.serial_no=Th.serialNumber
where Gs.sid=@id
end
else
begin
Select Th.* from NonGUCianStudentRegisterThesis NGS inner join Thesis Th on NGS.serial_no=Th.serialNumber
where NGS.sid=@id
end


--Examiner Components Procedures(Sara Salem)
go
...
go
create proc ViewAttended
@examinerid int
as
select Th.title, s.name,Gs.firstName
From Examiner e inner join ExaminerEvaluateDefense ee on e.id=ee.examinerId
inner join Thesis th on ee.serialNo=th.serialNumber
inner join GUCianStudentRegisterThesis Gt on th.serialNumber =Gt.serial_no
inner join Supervisor s on s.id= Gt.supid
inner join GucianStudent Gs on Gt.sid= Gs.id
where th.defenseDate<Convert(Date,CURRENT_TIMESTAMP)
union 
select Th.title, s.name, Ng.firstName
From Examiner e inner join ExaminerEvaluateDefense ee on e.id=ee.examinerId
inner join Thesis th on ee.serialNo=th.serialNumber
inner join NonGUCianStudentRegisterThesis Nt on th.serialNumber =Nt.serial_no
inner join Supervisor s on s.id= Nt.supid
inner join NonGucianStudent Ng on Nt.sid= Ng.id
where th.defenseDate<Convert(Date,CURRENT_TIMESTAMP)
go

go
alter proc Searchproc
@keyword varchar(50)
as
